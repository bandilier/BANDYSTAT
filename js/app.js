...
function setControls(){
    controls.forEach(b=>{
        b.disabled=!(controllerOnline&&!commandInProgress);
    });
}
...
function setConnectionState(online,showOverlay){
    ...
    if(online){
        ...
        el("connectionOverlayText").textContent=
            hasConnectedBefore
    ...
    el("diagnosticController").textContent=
        online?"Online":"Delayed";
...
function showMessage(message,type){
    ...
    if(type){
        ...
        commandMessageTimer=setTimeout(()=>{
            ...
            box.textContent=
                controllerOnline
                ?"BANDYSTAT is online and ready."
                :"Reconnecting to BANDYSTAT…";
...
function setButton(id,active,activeClass){
    ...
    if(active)
        b.classList.add("active",activeClass);
...
function displayThermostat(data){
    ...
    el("downstairsTemperature").textContent=
        num(data.downstairsTemp,1)+"°F";

    el("upstairsTemperature").textContent=
        num(data.upstairsTemp,1)+"°F";


    el("activeSensor").textContent=
        data.activeSensor
        ?"Sensor "+data.activeSensor
        :"--";
    ...
    el("equipmentDot").className=
        "equipment-dot "+eq.dot;


    el("systemBannerText").textContent=
        eq.banner;

    el("systemBanner").className=
        "system-banner "+eq.bannerClass;


    el("diagnosticSensor1").textContent=
        data.sensor1Error?"Error":"OK";

    el("diagnosticSensor2").textContent=
        data.sensor2Error?"Error":"OK";
}
...
async function refreshStatus(force=false){
    ...
    try{
        ...
        el("diagnosticUpdated").textContent=
            now;
        ...
        const seconds=
            ...
            ?(Date.now()-lastSuccessfulUpdate)/1000
            :999;
        ...
        if(offline)
            showMessage("Reconnecting to BANDYSTAT…");
...
async function sendCommand(command){
    ...
    if(!controllerOnline||commandInProgress)
        return;
...
function direction(p){
    ...
    if(!Number.isFinite(p))
        return "Pressure reading unavailable";


    if(p<-.05)
        return "House is negative";


    if(p>.05)
        return "House is positive";
...
function renderEvents(){
    ...
    if(!pressureEvents.length){
        ...
        box.innerHTML=
        '<div class="information-note">No pressure events recorded.</div>';
    ...
    box.innerHTML=
        ...
        '<div class="event-entry '+e.cls+'">'+
        '<span class="event-time">'+e.time+
        '</span><span>'+
...
function displayPressure(d){
    ...
    el("pressureValue").textContent=
        num(p,2);


    el("pressureDirection").textContent=
        direction(p);


    el("pressureState").textContent=
        style.name;


    el("pressureFan").textContent=
        d.fan||"--";


    el("pressureRequestedFan").textContent=
        d.requestedFan||"--";


    el("pressureRuntime").textContent=
        d.runtime;


    el("pressureStarts").textContent=
        Number.isFinite(d.starts)
        ?String(d.starts)
        :"--";


    el("pressureTemperature").textContent=
        Number.isFinite(d.temperature)
        ?num(d.temperature,1)+"°F"
        :"--.-°F";


    el("pressureHumidity").textContent=
    ...
    el("pressureBannerText").textContent=
        "Pressure controller "+
        style.name.toLowerCase();


    el("pressureBanner").className=
        "pressure-banner "+style.cls;



    el("pressureConnectionText").textContent=
        "Updated "+
        new Date().toLocaleTimeString();



    el("diagnosticPressureController").textContent=
        "Online";
    ...
    el("pressureGaugeNeedle").style.left=
        pct+"%";


    el("pressureGaugeLabel").textContent=
        num(p,2)+" Pa";
...
async function refreshPressure(){
    ...
    try{
        ...
        el("pressureBanner").className=
            "pressure-banner offline";


        el("pressureBannerText").textContent=
            "Pressure controller unavailable";
        ...
        pressureRefreshInProgress=false;
